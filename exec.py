...
class AppointmentForm(FlaskForm):
    appointment_date = DateField('Select a Date', validators=[DataRequired()])
    appointment_hour = SelectField('Select an Appointment Hour', validators=[DataRequired()])
...
def schedule_an_appointment(p_id, d_id):
    form = AppointmentForm()

    form.appointment_date.choices = [(appointment.date, appointment.date.strftime('%Y-%m-%d')) for appointment in Appointment.query.filter(
        db.and_(Appointment.doctor_id == d_id, Appointment.avaiable == True))]

    form.appointment_hour.choices = [(appointment.hour, appointment.hour) for appointment in Appointment.query.filter(
        db.and_(Appointment.doctor_id == d_id, Appointment.avaiable == True))]

    if form.submit.data:
        selected_date = form.appointment_date.data
        selected_hour = form.appointment_hour.data
        
        new_appointment = Appointment.query.filter(
            Appointment.date == selected_date,
            Appointment.hour == selected_hour).first()
        
        if new_appointment:
            if new_appointment.avaiable:
                new_appointment.patient_id = current_user.id
                new_appointment.avaiable = False
...
def update_appointment(id, a_id, d_id) -> str:
    form = AppointmentActionForm()
    form_2 = AppointmentForm()

    old_appointment = Appointment.query.get(a_id)

    form_2.appointment_date.choices = [(appointment.date, appointment.date.strftime('%Y-%m-%d')) for appointment in Appointment.query.filter(
        db.and_(Appointment.doctor_id == d_id, Appointment.avaiable == True))]

    form_2.appointment_hour.choices = [(appointment.hour, appointment.hour) for appointment in Appointment.query.filter(
        db.and_(Appointment.doctor_id == d_id, Appointment.avaiable == True))]

    if form.validate_on_submit():
        if form.submit.data:
            try:
                old_appointment.date = form.appointment_date.data
                old_appointment.hour = form.appointment_hour.data
...
@login_required
@app.route("/admin_view/reported_users", methods=["GET", "POST"])
def admin_view_reported_users() -> str:
    #
    pass
    ...
    while x != row_amount:
        for _ in range(5):
            for _ in range(8):
                ...
                db.session.commit()
            a -= 8
            todays_date += datetime.timedelta(hours = 24)
        break

def working_hours_gen(d_id, date) -> list:
    """ Extracts, verifies and returns list of avaiable hours for appointment with specific doctor."""
    ...
    for v in range(a, b):
        appointments = Appointment.query.filter(
            doctor_id = d_id,
            date = date).all()
        if appointments.avaiable == True:
            hour_string = f"{v}-{v + 1}"
            tuple_hour = (hour_string, hour_string)
            choices.append(tuple_hour)
